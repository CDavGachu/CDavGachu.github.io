const loadHome = async () => {
    ...
    .then(async (result) => {
        ...
        if(message.includes("Error") || message.includes("token has expired")){
            ...
            sessionStorage.setItem("email", userData.UserAttributes.find(attribute => attribute.Name == "email").Value);
            
            loadNotas();
                name.innerHTML = sessionStorage.getItem("given_name") + " " + sessionStorage.getItem("family_name");
                firstName.value = sessionStorage.getItem("given_name");
                lastName.value = sessionStorage.getItem("family_name");
