...
const submitAppointment = async () => {
    ...
    await fetch(`https://fz853w9zuj.execute-api.us-east-2.amazonaws.com/dev/citas?idUser=${sessionStorage.getItem("user")}&paciente=${localStorage.getItem("given_name")}+${localStorage.getItem("family_name")}&fecha_hora=${getISODate(hourToDate, dateToDate, '-06:00')}&duracion=${duration}`, requestOptions)
        .then(response => response.json())
        .then(async result => {
            console.log(result)
            if (result.message)
